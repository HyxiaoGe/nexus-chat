...
export const FullscreenAgentView: React.FC<FullscreenAgentViewProps> = ({
    ...
    const handleKeyDown = (e: KeyboardEvent) => {
      if (e.key === 'Escape') {
        ...
        const prevMessage = allMessages.find(
          (m) => m.agentId === prevAgent.id && m.role === 'model'
        );
        if (prevMessage) {
        ...
        const nextMessage = allMessages.find(
          (m) => m.agentId === nextAgent.id && m.role === 'model'
        );
        if (nextMessage) {
  ...
  }, [currentIndex, allAgents, allMessages, onNavigate, prevAgent, nextAgent, handleClose, handleCopy]);
      ...
      <main className="flex-1 overflow-y-auto">
        <div className="max-w-4xl mx-auto px-6 py-12">
          ...
          <div className="prose prose-lg dark:prose-invert max-w-none">
            <SmartContentRenderer content={message.content} isStreaming={false} />
          </div>
              ...
              <div className="flex items-center justify-between">
                ...
                {message.tokenUsage.estimatedCost && message.tokenUsage.estimatedCost > 0 && (
                  <div className="text-right">
                    ...
                    <div className="text-xs text-gray-500 dark:text-gray-400">预估成本</div>
                  </div>
                )}
              </div>
            </div>
                ...
                <button
                  ...
                  onClick={() => {
                    const msg = allMessages.find(
                      (m) => m.agentId === agent.id && m.role === 'model'
                    );
                    if (msg) {
