    ...
    <div className="my-4 rounded-lg overflow-hidden border border-gray-300 dark:border-gray-700 shadow-sm bg-gray-50 dark:bg-[#0d1117] group">
      <div className="flex items-center justify-between px-4 py-2 bg-gray-200 dark:bg-gray-800 border-b border-gray-300 dark:border-gray-700">
        <div className="flex items-center gap-2">
          ...
          <span className="text-xs font-bold font-mono text-gray-600 dark:text-gray-300 uppercase">
            {language || t('common.text')}
          </span>
        </div>
        <button
          onClick={handleCopy}
...
export const SmartContentRenderer: React.FC<SmartContentRendererProps> = ({
  ...
  useEffect(() => {
    if (isThinkingActive && !isThinkingOpen) {
      // This setState is intentional to sync state with prop changes
      // eslint-disable-next-line react-hooks/set-state-in-effect
      setIsThinkingOpen(true);
    }
  }, [isThinkingActive, isThinkingOpen]);
    ...
    return parts.map((part, index) => {
      ...
      if (part.startsWith('$$') && part.endsWith('$$')) {
        ...
        return (
          <div
            key={index}
            className="my-4 p-4 bg-blue-50 dark:bg-blue-900/10 border border-blue-200 dark:border-blue-800 rounded-lg overflow-x-auto"
          >
      ...
      while (lineIdx < lines.length) {
        ...
        if (line.startsWith('### ')) {
          elements.push(
            <h3
              key={lineIdx}
              className="text-lg font-bold mt-4 mb-2 text-gray-900 dark:text-white tracking-tight"
            >
        ...
        if (line.startsWith('## ')) {
          elements.push(
            <h2
              key={lineIdx}
              className="text-xl font-bold mt-6 mb-3 border-b border-gray-200 dark:border-gray-700 pb-2 text-gray-900 dark:text-white tracking-tight"
            >
              {parseInlineStyles(line.replace('## ', ''))}
            </h2>
          );
        ...
        if (line.startsWith('> ')) {
          elements.push(
            <div
              key={lineIdx}
              className="flex gap-3 pl-4 border-l-4 border-blue-300 dark:border-blue-600/50 italic text-gray-600 dark:text-gray-400 my-2 py-1 bg-gray-50 dark:bg-gray-900/30 rounded-r"
            >
        ...
        if (taskMatch) {
          ...
          elements.push(
            <div key={lineIdx} className="flex gap-2 ml-2 items-start">
              ...
              />
              <div
                className={`flex-1 ${isChecked ? 'line-through text-gray-500 dark:text-gray-600' : ''}`}
              >
        ...
        if (numMatch) {
          elements.push(
            <div key={lineIdx} className="flex gap-2 ml-2 items-start">
              <span className="font-mono text-gray-500 font-bold min-w-[1.5rem] text-right">
                {numMatch[1]}.
              </span>
    ...
    return mathParts.map((mathPart, i) => {
      if (mathPart.startsWith('$') && mathPart.endsWith('$') && mathPart.length > 2) {
        ...
        return (
          <span
            key={i}
            className="inline-block bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 px-2 py-0.5 rounded text-sm font-serif italic text-blue-800 dark:text-blue-300 mx-1"
          >
            {mathContent}
          </span>
        );
      ...
      return codeParts.map((codePart, j) => {
        if (codePart.startsWith('`') && codePart.endsWith('`')) {
          return (
            <code
              key={`${i}-${j}`}
              className="bg-gray-100 dark:bg-gray-800 border border-gray-200 dark:border-gray-700 px-1.5 py-0.5 rounded text-sm font-mono text-pink-600 dark:text-pink-400 break-all"
            >
              {codePart.slice(1, -1)}
            </code>
          );
    ...
    <div className="space-y-4">
      ...
      {thinkingContent && (
        ...
        >
          ...
          >
            <div
              className={`p-1 rounded-md ${isThinkingActive ? 'bg-purple-200 dark:bg-purple-800 text-purple-700 dark:text-purple-300' : 'bg-gray-200 dark:bg-gray-700 text-gray-500 dark:text-gray-400'}`}
            >
            ...
            <div className="flex-1 flex items-center gap-2">
              <span
                className={`uppercase tracking-wider text-[10px] font-bold ${isThinkingActive ? 'text-purple-700 dark:text-purple-300' : 'text-gray-500 dark:text-gray-400'}`}
              >
          ...
          {isThinkingOpen && (
            <div className="p-4 text-xs leading-relaxed text-gray-600 dark:text-gray-400 font-mono whitespace-pre-wrap animate-in slide-in-from-top-2 duration-200 max-h-[500px] overflow-y-auto custom-scrollbar bg-white/50 dark:bg-black/20">
              {thinkingContent}
              {isThinkingActive && (
                <span className="animate-pulse ml-0.5 inline-block w-1.5 h-3 bg-purple-400 align-middle"></span>
              )}
            </div>
          )}
        </div>
      ...
      {/* Main Content */}
      <div className="min-w-0 markdown-body">{renderRichText(mainContent)}</div>
    </div>
  );
};
